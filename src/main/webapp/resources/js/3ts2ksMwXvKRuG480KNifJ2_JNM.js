...
(function() {
	...
	var noteError = function(options) {
		var hash = options.selector + '::' + options.type + '::'
				+ (options.installId || '');
		if (errorHashes[hash])
			return;
	...
	CloudflareApps.createElement = function(options, prevEl) {
		...
		try {
			...
			if (!container.parentNode
					&& (options.method == "after" || options.method == "before" || options.method == "replace")) {
				return element;
			}
			...
			case "after":
				if (container.nextSibling) {
					container.parentNode.insertBefore(element,
...
(function() {
	CloudflareApps.matchPage = function(patterns) {
		...
		if (window.CloudflareApps && CloudflareApps.proxy
